...
class DB_Manager {
    ...
    public func addUser(nameValue: String, emailValue: String, ageValue: Int64, noteValue: String) {
        do {
    ...
    public func updateUser(idValue: Int64, nameValue: String, emailValue: String, ageValue: Int64, noteValue:String) {
        do {
            // get user using ID
            let user: Table = users.filter(id == idValue)
             
            // run the update query
            try db.run(user.update(name <- nameValue, email <- emailValue, age <- ageValue,note <- noteValue))
        } catch {
            print(error.localizedDescription)
        }
    }
